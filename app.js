...
function getHueFromHex(hex) {
    const [r, g, b] = hex.match(/[a-f0-9]{2}/gi).map(x => parseInt(x, 16) / 255);
...
async function fetchAndRenderPalettes() {
    try {
        ...
        renderPalettes([{ id: 1, name: "Default Palette", colors: ["#FF0000", "#00FF00", "#0000FF", "#BF39A9"] }]); // Uncomment this line if you have a default state to show
        // renderPalettes([]);
        // return []; 
    }
}
        ...
        return `
            <div class="color-palette" data-id="${palette.id}">
                ...
                <div class="palette-actions">
                    <button class="edit-btn" data-id="${palette.id}"  title ='Edit Palette'>Edit</button>
                    <button class="delete-btn" data-id="${palette.id}"  title ='Delete Palette'>Delete</button>
...
function handleEdit(e) {
    ...
    currentlyEditingId = palette.dataset.id;
    
    paletteNameInput.value = palette.querySelector("h5").textContent;
    selectedColors = Array.from(palette.querySelectorAll(".color p")).map(el => el.textContent);
...
async function addPalette() {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Failed to add palette");
        resetForm();
        fetchAndRenderPalettes();
    } catch (error) {
        handleError("adding palette", error);
    }
}
...
async function updatePalette() {
    ...
    try {
        const response = await fetch(getApiUrl(currentlyEditingId), {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ 
                name: paletteNameInput.value.trim(),
        ...
        handleError("updating palette", error);
    }
}
...
async function handleDelete(e) {
    ...
    try {
        const response = await fetch(getApiUrl(e.target.dataset.id), {
            method: "DELETE"
        });
        ...
        handleError("deleting palette", error);
    }
}
